...
$Output = Join-Path $Root "dist-arm-wrapper-v22beta-networktest5-dyn14-exact-network"
...
$BuilderRevision = "dynarmic-x64-builder34-v22beta-networktest5-dyn14-exact-network"
...
if ($InputExtension -ne ".apk") {
    throw "NetworkTest5 accepts an APK input, not a raw .so or donor package: $ResolvedApk"
}
...
$V22Notes = Join-Path $Root "NETWORKTEST5-NOTES.md"
if (Test-Path $V22Notes) { Copy-Item -Force $V22Notes (Join-Path $Output "NETWORKTEST5-NOTES.md") }
...
GeometryDashDynarmicProbe.exe game-v22beta-selected.apk --companion-hooks=off --debug-everything --dump-imports=gd-networktest5-imports.txt --log=gd-networktest5.log --profile=gd-networktest5-profile.csv --profile-summary=gd-networktest5-profile-summary.txt
set "RESULT=%ERRORLEVEL%"
echo.
echo Main log: gd-networktest5.log
echo Imports: gd-networktest5-imports.txt
pause
exit /b %RESULT%
'@
[IO.File]::WriteAllText((Join-Path $Output "RUN_NETWORKTEST5.cmd"), $SelectedLauncher, [Text.Encoding]::ASCII)
...
Write-Host "`nDynarmic x64 2.2 beta ARMv7 NetworkTest5 branch ready:" -ForegroundColor Green
Write-Host "  $Output"
Write-Host "Run: RUN_NETWORKTEST5.cmd"
