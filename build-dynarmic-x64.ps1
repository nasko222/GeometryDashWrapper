...
$Output = Join-Path $Root "dist-arm-wrapper-v22beta-networktest6-safe-async-worker"
...
$BuilderRevision = "dynarmic-x64-builder34-v22beta-networktest6-safe-async-worker"
...
if ($InputExtension -ne ".apk") {
    throw "NetworkTest6 accepts an APK input, not a raw .so or donor package: $ResolvedApk"
}
...
GeometryDashDynarmicProbe.exe game-v22beta-selected.apk --companion-hooks=off --debug-everything --dump-imports=gd-networktest6-imports.txt --log=gd-networktest6.log --profile=gd-networktest6-profile.csv --profile-summary=gd-networktest6-profile-summary.txt
set "RESULT=%ERRORLEVEL%"
echo.
echo Main log: gd-networktest6.log
echo Imports: gd-networktest6-imports.txt
pause
exit /b %RESULT%
'@
...
Write-Host "`nDynarmic x64 2.2 beta ARMv7 NetworkTest6 branch ready:" -ForegroundColor Green
Write-Host "  $Output"
