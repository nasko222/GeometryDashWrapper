...
$Output = Join-Path $Root "dist-arm-wrapper-v22beta-networktest7-native-winhttp-bridge"
...
$BuilderRevision = "dynarmic-x64-builder35-v22beta-networktest7-native-winhttp-bridge"
...
if ($InputExtension -ne ".apk") {
    throw "NetworkTest7 accepts an APK input, not a raw .so or donor package: $ResolvedApk"
}
...
GeometryDashDynarmicProbe.exe game-v22beta-selected.apk --companion-hooks=off --debug-everything --dump-imports=gd-networktest7-imports.txt --log=gd-networktest7.log --profile=gd-networktest7-profile.csv --profile-summary=gd-networktest7-profile-summary.txt
set "RESULT=%ERRORLEVEL%"
echo.
echo Main log: gd-networktest7.log
echo Imports: gd-networktest7-imports.txt
pause
exit /b %RESULT%
'@
...
Write-Host "`nDynarmic x64 2.2 beta ARMv7 NetworkTest7 branch ready:" -ForegroundColor Green
Write-Host "  $Output"
